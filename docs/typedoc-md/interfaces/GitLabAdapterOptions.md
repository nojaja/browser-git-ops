[browser-git-ops - v0.0.7](../README.md) / GitLabAdapterOptions

# Interface: GitLabAdapterOptions

## Hierarchy

- [`AdapterOptionsBase`](AdapterOptionsBase.md)

  ↳ **`GitLabAdapterOptions`**

## Table of contents

### Properties

- [branch](GitLabAdapterOptions.md#branch)
- [defaultBranch](GitLabAdapterOptions.md#defaultbranch)
- [host](GitLabAdapterOptions.md#host)
- [projectId](GitLabAdapterOptions.md#projectid)
- [repositoryId](GitLabAdapterOptions.md#repositoryid)
- [repositoryName](GitLabAdapterOptions.md#repositoryname)
- [token](GitLabAdapterOptions.md#token)

## Properties

### branch

• `Optional` **branch**: `string`

#### Inherited from

[AdapterOptionsBase](AdapterOptionsBase.md).[branch](AdapterOptionsBase.md#branch)

#### Defined in

[virtualfs/types.ts:24](https://github.com/nojaja/browser-git-ops/blob/c51cf29673a6f5165e49aea29ccb5792f2611947/src/virtualfs/types.ts#L24)

___

### defaultBranch

• `Optional` **defaultBranch**: `string`

#### Inherited from

[AdapterOptionsBase](AdapterOptionsBase.md).[defaultBranch](AdapterOptionsBase.md#defaultbranch)

#### Defined in

[virtualfs/types.ts:26](https://github.com/nojaja/browser-git-ops/blob/c51cf29673a6f5165e49aea29ccb5792f2611947/src/virtualfs/types.ts#L26)

___

### host

• `Optional` **host**: `string`

#### Inherited from

[AdapterOptionsBase](AdapterOptionsBase.md).[host](AdapterOptionsBase.md#host)

#### Defined in

[virtualfs/types.ts:25](https://github.com/nojaja/browser-git-ops/blob/c51cf29673a6f5165e49aea29ccb5792f2611947/src/virtualfs/types.ts#L25)

___

### projectId

• **projectId**: `string`

#### Defined in

[virtualfs/types.ts:37](https://github.com/nojaja/browser-git-ops/blob/c51cf29673a6f5165e49aea29ccb5792f2611947/src/virtualfs/types.ts#L37)

___

### repositoryId

• `Optional` **repositoryId**: `string` \| `number`

#### Inherited from

[AdapterOptionsBase](AdapterOptionsBase.md).[repositoryId](AdapterOptionsBase.md#repositoryid)

#### Defined in

[virtualfs/types.ts:28](https://github.com/nojaja/browser-git-ops/blob/c51cf29673a6f5165e49aea29ccb5792f2611947/src/virtualfs/types.ts#L28)

___

### repositoryName

• `Optional` **repositoryName**: `string`

#### Inherited from

[AdapterOptionsBase](AdapterOptionsBase.md).[repositoryName](AdapterOptionsBase.md#repositoryname)

#### Defined in

[virtualfs/types.ts:27](https://github.com/nojaja/browser-git-ops/blob/c51cf29673a6f5165e49aea29ccb5792f2611947/src/virtualfs/types.ts#L27)

___

### token

• `Optional` **token**: `string`

#### Inherited from

[AdapterOptionsBase](AdapterOptionsBase.md).[token](AdapterOptionsBase.md#token)

#### Defined in

[virtualfs/types.ts:23](https://github.com/nojaja/browser-git-ops/blob/c51cf29673a6f5165e49aea29ccb5792f2611947/src/virtualfs/types.ts#L23)
